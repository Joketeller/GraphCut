...
def MeanShift(img, _h_value=120, _iter_value=30):
    ...
    while rgb_array.shape[0] != 0:
        ...
        for i in range(0, rgb_array.shape[0]):
            euclid_dist = ((rgb_array[i][0] - meanr) ** 2 + (rgb_array[i][1] - meang) ** 2 + (
                    rgb_array[i][2] - meanb) ** 2 + (rgb_array[i][3] - meanx) ** 2 + (
                                   rgb_array[i][4] - meany) ** 2) ** 0.5
        ...
        if len(less_than_h) > 0:
            ...
            mean_for_first_element, mean_for_second_element, mean_for_third_element, mean_for_fourth_element, mean_for_fifth_element = mean_for_first_element / len(
                less_than_h), mean_for_second_element / len(less_than_h), mean_for_third_element / len(
                less_than_h), mean_for_fourth_element / len(less_than_h), mean_for_fifth_element / len(less_than_h)

            new_mean_value_lesser = [mean_for_first_element, mean_for_second_element, mean_for_third_element,
                                     mean_for_fourth_element, mean_for_fifth_element]

            new_euclid_dist = ((mean_for_first_element - meanr) ** 2 + (mean_for_second_element - meang) ** 2 + (
                    mean_for_third_element - meanb) ** 2 + (mean_for_fourth_element - meanx) ** 2 + (
                                       mean_for_fifth_element - meany) ** 2) ** 0.5
